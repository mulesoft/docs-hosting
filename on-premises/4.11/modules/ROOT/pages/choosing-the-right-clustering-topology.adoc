...
A server group is not a cluster. A server group is a set of unclustered servers that Runtime Manager can deploy to together. A cluster shares memory, VM queues, and object-store data across nodes and requires the Enterprise standalone distribution.
