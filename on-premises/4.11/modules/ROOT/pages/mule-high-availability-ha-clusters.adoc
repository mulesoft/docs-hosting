...
An on-premises Mule runtime cluster does not create a virtual IP. An on-premises cluster does not include a built-in load balancer. You supply a third-party load balancer or client-side failover for HTTP and TCP traffic. Clusters do not distribute application artifacts to nodes; you deploy the same application to each node or through Runtime Manager.
