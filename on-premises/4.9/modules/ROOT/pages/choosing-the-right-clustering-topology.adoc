...
A server group isn't a cluster. A server group is a set of unclustered servers that Runtime Manager can deploy to together. Server groups and Runtime Manager clusters are in xref:hybrid-standalone::index.adoc[Hybrid Standalone Runtimes]. See the Cluster stage in xref:index.adoc[Run Mule Runtime Engine On-Premises].
