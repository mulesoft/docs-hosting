= Create and Manage an On-Premises Mule Runtime Cluster Manually
:page-aliases: 4.12@mule-runtime::creating-and-managing-a-cluster-manually.adoc
...
Create a cluster after you install Mule Enterprise Edition, install an xref:installing-an-enterprise-license.adoc[Enterprise license], and choose a topology. See xref:choosing-the-right-clustering-topology.adoc[Cluster On-Premises Mule Runtime Instances] and xref:mule-high-availability-ha-clusters.adoc[On-Premises Mule Runtime High Availability Cluster Overview]. The cluster doesn't include a load balancer. You still start and stop each node with xref:starting-and-stopping-mule-esb.adoc[Start and Stop On-Premises Mule Runtime Engine].

There are two ways to create and manage clusters for on-premises runtime:
...
== Review Cluster Requirements and Restrictions

* Don't mix cluster management tools.
+
...
** Mule runtime engine
+
If you're using a cumulative patch release, such as 4.3.0-20210322, all instances of Mule must be the same cumulative patch version.
...
. Ensure that the node isn't running, that is that the Mule Runtime Server is stopped.
...
For the full list of available parameters, see <<cluster-config-params>>.

== Manage a Cluster Manually
...
When partitioning a network, clusters are available by default. However, by setting a minimum quorum size you can configure your cluster to reject updates that don't pass a minimum threshold. +
...
* The remaining members of the cluster become so overwhelmed with requests that they’re unable to respond and are forced out of the cluster on the assumption that they're dead.
...
=== Persist Object Stores

Persistent object stores aren't shared across a full cluster shutdown unless you configure a centralized JDBC store. The in-grid copy doesn't survive stopping every node, even when the object store `persistent` flag is true.
...
You can persistently store JDBC data in a central system that's accessible by all cluster nodes when using Mule runtime engine on-premises.

These relational database systems are supported:
...
* Don't share the same database across different clusters. Because clusters aren't aware of each other, sharing a database can lead to data corruption, for example when Mule applications in different clusters have the same name. You can reuse the same database when you're migrating a single cluster to a new runtime minor version, but stop all cluster nodes until the migration finishes to prevent serialization errors. See xref:mule-runtime::mule-upgrade-tool.adoc#upgrade_clustered_mule_instances[Upgrading Clustered Mule Instances] for the upgrade procedure.
...
* The persistent object store uses a database connection pool based on the `ComboPooledDataSource` Java class. The Mule runtime engine doesn't set any explicit values for the connection pool behavior. The standard configuration uses the default value for each property. +
For example, the default value for `maxIdleTime` is 0, which means that idle connections never expire and aren't removed from the pool. Idle connections remain connected to the database in an idle state. +
You can configure the connection pool behavior by passing your desired parameter values to the runtime, using either of these options:
...
+
[NOTE]
You can find more information about the pool configuration of the `ComboPooledDataSource` Java class in https://www.mchange.com/projects/c3p0/[this article].

=== Monitor the Cluster

You can monitor the events thrown by the cluster members through the https://www.oracle.com/java/technologies/javase/javamanagement.html[JMX technology].
...
For example, if you set this property in the `mule-cluster.properties` file, Mule hashes table names using MD5 and a prefix to identify tables as Mule tables:
[source]
----
...
== Set mule-cluster.properties Parameters

This table lists the parameters of the `mule-cluster.properties` file.
...
This option configures the cluster with the specified fixed IP addresses. Use this option if you aren't relying on multicast for cluster node discovery. If using this option, set `mule.cluster.multicastenabled` to `false`, otherwise an exception is thrown when you start the cluster. (see next item in this table). +
...
If set to `true`, don't set IP addresses in `mule.cluster.nodes`.  | No
...
The manual cluster file is invalid or disagrees with other nodes. `mule.clusterId` and `mule.clusterNodeId` must be present. Unicast (`mule.cluster.nodes`) requires `mule.cluster.multicastenabled=false`. Multicast isn't available on Amazon Web Services; use unicast there.

=== `NoDataMemberInClusterException`

The cluster has no Hazelcast data member for the requested partition. Check quorum (`mule.cluster.quorumsize`) and that enough nodes are running.
...
* xref:runtime-manager::cluster-about.adoc[Clusters]
