= Harden Your On-Premises Mule Runtime Engine Installation
:page-aliases: 4.12@mule-runtime::hardening-your-mule-installation.adoc
:keywords: hardening, FIPS, TLS, cipher suites, keystore, BCFKS, non-root, file permissions, on-premises, on-prem, standalone

Harden on-premises runtime after Mule applications deploy successfully and before you use the instance in production. Run as a non-root user, install as a service, restrict file permissions, and configure TLS or FIPS. This is the Harden stage in xref:index.adoc[On-Premises Runtimes Overview].

Use secure configuration files for passwords and keystores. Keep those files outside the Mule application archive when DevOps must change them without a redeploy.

== Follow the Hardening Checklist

To harden on-premises runtime:

* Run Mule as a non-privileged user.
* Install Mule as a service. See xref:starting-and-stopping-mule-esb.adoc[Start and Stop On-Premises Mule Runtime Engine].
* Configure Mule to write logs or temporary files in appropriate locations. Configure logs, passwords, and keystore files.
* Make usernames, passwords, and keystores available externally so DevOps can change them.
* Manage certificates in a keystore file.
* Use a separate property file to store usernames and passwords, and secure it using file system permissions.
* For FIPS, see xref:mule-runtime::fips-140-2-compliance-support.adoc[FIPS 140-2 Compliance Support] and xref:installing-an-enterprise-license.adoc#download-your-license-key[Download Your License Key].

=== Run as a Non-Root User

Run on-premises Mule runtime engine as a non-root OS user. Root isn't required except to bind ports below 1024. The runtime user must own `$MULE_HOME` and have write permission to the logs directory.


On Linux and Unix, you can run Mule as any user with these caveats:

* You need write permissions to logs directory
* Without root you cannot use ports below 1024

On Windows, you need to be in admin user's group to run Mule.

== Harden in Layers

By its nature, you can situate Mule in a variety of configurations. The suggested approach to hardening involves hardening in layers beginning with the operating system, then working up the stack. The Center for Internet Security (CIS) publishes configuration benchmarks that are widely used in whole or in part as system hardening guides.

Commercial configuration management and integrity management tools can help you automate management to the CIS benchmarks. Also, Mule documentation includes a good deal of information on configuring security. If your application deals in sensitive data, consider using SSL Transport (HTTPS) to protect it.

On the network security side, security experts recommend using a good stateful inspection network firewall with a default-deny rule set and exceptions only for justified business needs. Also, any internet facing server belongs in a DMZ with strong default-deny egress rules on the firewall to prevent data exfiltration. Furthermore, you can use a network IDS/IPS to monitor and prevent known attacks. Putting the database on an internal network - not the DMZ - also helps to harden your installation.

Be sure your software developers are familiar with secure web application coding techniques. At the very least, they should be familiar with best practices to avoid common web app pitfalls, such as those listed in OWASP's top 10 Project.



== Troubleshoot Common Error Messages

=== `PKIX path building failed`

TLS trust failed while the on-premises runtime or Runtime Manager agent (`amc_setup`) connected to an HTTPS endpoint. Import the issuing CA into the JVM truststore or configure the corporate proxy certificate. This error isn't a Mule application flow failure.

== See Also

* xref:index.adoc[On-Premises Runtimes Overview]
* xref:starting-and-stopping-mule-esb.adoc[Start and Stop On-Premises Mule Runtime Engine]
* xref:mule-deployment-model.adoc[On-Premises Mule Runtime Deployment Model]
* xref:mule-runtime::fips-140-2-compliance-support.adoc[FIPS 140-2 Compliance Support]
* xref:mule-runtime::securing.adoc[Security]
* xref:runtime-fabric::configure-hardening.adoc[Harden Mule Runtime on Runtime Fabric]
