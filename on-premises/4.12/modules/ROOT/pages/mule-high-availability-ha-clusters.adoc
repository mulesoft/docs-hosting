= On-Premises Mule Runtime High Availability Cluster Overview
:page-aliases: 4.12@mule-runtime::mule-high-availability-ha-clusters.adoc
:keywords: hazelcast, quorum, split brain, node failure, failover, virtual IP, load balancer, server group, object store, mule-cluster.properties, 5701, multicast, unicast, on-premises, on-prem, standalone, ha, high availability, downtime

Mule Enterprise Edition supports scalable clustering to provide high availability (HA) for on-premises Mule applications. A cluster groups multiple Mule runtime engine nodes into a single unit that shares memory, VM queues, and Object Store data across a distributed grid. Clustering enables automatic failover when a node becomes unavailable, load balancing across active nodes, and coordinated access to resources like files and databases.

Clustering is optional. Complete license, start, and deploy first. See the Cluster stage in xref:index.adoc[On-Premises Runtimes Overview]. Choose a topology in xref:choosing-the-right-clustering-topology.adoc[Cluster On-Premises Mule Runtime Instances] before you create the cluster. You can configure clusters through Anypoint Runtime Manager after you register the Runtime Manager agent, or manually for unregistered on-premises runtime.
...
== Review Clustering Benefits

An on-premises Mule runtime cluster doesn't create a virtual IP. An on-premises cluster doesn't include a built-in load balancer. You supply a third-party load balancer or client-side failover for HTTP and TCP traffic. Clusters don't distribute application artifacts to nodes; you deploy the same application to each node or through Runtime Manager.
...
image::failovernofail.png["A failover process with multiple nodes and a client"]
...
image::failovernode2fail.png["A failover process with two nodes and a client in a cluster"]

Because all nodes in a cluster of Mule runtime engines are "active-active" and can process messages simultaneously, clusters can also improve performance and scalability. Compared to a single node instance, clusters can support more users or improve application performance by sharing the workload across multiple nodes or by adding nodes to the cluster. Note that not all Mule applications perform better when horizontally scaling or clustering. Performance depends on the nature of the work to be shared with additional nodes. Performance of some Mule applications with a heavy dependency on Object Stores can degrade because of the work required to replicate or coordinate access to the data within the shared memory grid.
...
=== Resolve Concurrency Issues with Clusters
...
image::load-balancing.png["A four-node Mule cluster designed for load balancing with various flow processes"]

== Build High-Reliability Applications
...
[NOTE]
Transactions can't span interactions or operations with systems for which the connector doesn't support transactions. If any involved operations can't be included in the transaction, consider an alternative pattern such as https://www.cs.cornell.edu/andru/cs711/2002fa/reading/sagas.pdf[Sagas].

== Use Connectors in a Cluster
...
== Build Reliable Applications in a Cluster

High-reliability Mule applications must have zero tolerance for message loss. This means that both the underlying Mule and its individual connections need to be reliable. You can find tools to build fully reliable Mule applications in your clusters in xref:mule-runtime::reliability-patterns.adoc[Reliability Patterns].
...
If your application uses transactional connectors, such as JMS, VM, and JDBC, use transactions. Mule's built-in support for transactional connectors enables reliable messaging for Mule applications that use these connectors.

These actions can also apply to non-clustered Mule applications.

== Network an On-Premises Cluster

=== Cluster in a Single Data Center
...
Various software load balancers are available, including:
...
== Configure a Cluster for High Performance
...
If high performance is your primary goal (rather than reliability), you can configure a Mule cluster or an individual application for maximum performance using a performance profile. By implementing the performance profile for specific Mule applications within a cluster, you can maximize the scalability of your deployments while deploying Mule applications with different performance and reliability requirements in the same cluster. Performance profiles that you configure at the container level apply to all Mule applications within the container. Application-level configuration overrides container-level configuration.

=== Review Performance Profile Considerations
...
To configure the performance profile at the _container_ level, add this to `mule-cluster.properties`, to the system properties (from the command line), or to `wrapper.conf`:
...
xsi:schemaLocation="
...
http://www.mulesoft.org/schema/mule/ee/cluster https://www.mulesoft.org/schema/mule/ee/cluster/current/mule-cluster.xsd
...
>
   <configuration>
...
Remember that an application-level configuration overrides a container-level configuration. If you want to configure the container for high performance but make one or more individual Mule applications within that container prioritize reliability, include this code in those Mule applications:
...
See xref:mule-runtime::fips-140-2-compliance-support.adoc[FIPS-Compliant Support] for configuration instructions.
* Encrypt communication between cluster nodes. +
Define the `mule.cluster.network.encryption.key` property, either in the `wrapper.conf` file or as a system property. Valid key lengths are 16, 24, or 32 bytes.

=== Review FIPS Clustering Warnings
...
== Follow Clustering Best Practices
...
* If your application processes messages from a non-transactional connector, use a xref:mule-runtime::reliability-patterns.adoc[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Use the VM connector to get optimal performance. Use the JMS connector for Mule applications where data needs to be saved after the entire cluster exits.
* Implement reliability patterns to create high-reliability Mule applications.
* In HA cluster mode, all Object Store content, both in-memory and persistent (as defined by the `persistent` parameter), is stored in the distributed memory grid. As long as the cluster maintains quorum, this data survives application redeploys or restarts. To fully clear the Object Store content, all cluster nodes must be stopped before restarting. For scenarios where data persistence is required even after a full cluster shutdown, configure the Object Store to use a JDBC-based store instead of the memory grid. See xref:creating-and-managing-a-cluster-manually.adoc#object-store-persistence[Object Store Persistence].
...
The node rejected `mule-cluster.properties` or could not join the Hazelcast cluster. Confirm every node uses the same Mule runtime engine version, Java version, and Runtime Manager agent version, and that TCP port `5701` (and `5702`/`5703` if you use the Runtime Manager defaults) is open between nodes.

=== `NoDataMemberInClusterException`

Hazelcast has no data member that can serve a partition. This occurs after node loss, split brain, or a quorum that is no longer met. Restore enough nodes or reduce `mule.cluster.quorumsize` only if you accept weaker consistency.
...
* xref:cloudhub-2::ch2-clustering.adoc[Clustering in CloudHub 2.0]
