= High Availability and Disaster Recovery for On-Premises Mule Runtime Engine
:page-aliases: 4.12@mule-runtime::on-premises-hadr.adoc
:keywords: high availability, disaster recovery, failover, RTO, RPO, backup, active-active, active-passive, data center, on-premises, on-prem, standalone
...
HA clustering is optional and requires an xref:installing-an-enterprise-license.adoc[Enterprise license]. Choose a topology in xref:choosing-the-right-clustering-topology.adoc[Cluster On-Premises Mule Runtime Instances] before you size standby capacity. See the Cluster stage in xref:index.adoc[On-Premises Runtimes Overview].
...
== Compare High Availability and Disaster Recovery

High availability (HA) measures a system's ability to remain accessible when a component fails. On on-premises servers, you achieve HA by clustering and load balancing Mule runtime nodes, so that another node takes over when one fails.
...
== Achieve High Availability On-Premises

You achieve high availability on on-premises servers by clustering and load balancing Mule runtime nodes. Depending on your SLA, you choose a standby topology and a deployment architecture that meet your availability and fault-tolerance requirements.
...
=== Choose an On-Premises Deployment Model
...
Because you control the infrastructure, you own disaster recovery for on-premises deployments. You back up your servers, Mule applications, and data, and you bring backup infrastructure online to meet your recovery objectives when your primary environment fails.
...
Simultaneous node restarts:: Don't restart multiple cluster nodes at the same time, such as during operating system patching. Concurrent restarts can trigger cluster-membership eviction and take Mule applications offline.
...
== Follow High Availability and Disaster Recovery Best Practices

Follow these design principles for high availability and disaster recovery on on-premises servers:
