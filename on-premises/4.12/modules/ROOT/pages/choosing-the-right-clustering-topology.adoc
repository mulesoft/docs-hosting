...
You can deploy Mule in many different topologies. As you build your Mule application, it is important to think critically about how best to architect your application to achieve the desired availability, fault tolerance, and performance characteristics. This page outlines some of the solutions for achieving the right blend of these characteristics through clustering when you deploy Mule applications on-premises. There is no one correct approach for everyone, and designing your system is both an art and a science. If you need more assistance, MuleSoft Professional Services can help you by reviewing your architecture plan or designing it for you. For more information, https://www.mulesoft.com/contact[contact us].

== Choose a Clustering Topology

A server group isn't a cluster. A server group is a set of unclustered servers that Runtime Manager can deploy to together. A cluster shares memory, VM queues, and object-store data across nodes and requires the Enterprise standalone distribution.


Deploying an application into a cluster is useful for achieving these:
