...
Download, unpack, and configure on-premises runtime after you confirm xref:hardware-and-software-requirements.adoc[Hardware and Software Requirements for On-Premises Mule Runtime Engine]. This is the Download and Wrapper stages in xref:index.adoc[Run Mule Runtime Engine On-Premises]. After the JVM starts in the console, stop it and install an xref:installing-an-enterprise-license.adoc[Enterprise license] before you run the instance in production.

Before downloading and installing Mule runtime engine, verify that you have a supported JDK installed.
This example uses https://adoptium.net/temurin/releases/?version=8[Eclipse Temurin 8^], which is recommended for Mule runtime engine 4.6.
...
+
Example for Mule runtime engine 4.6.0 in the `Downloads` directory:
+
* In Windows environments:
+
[source,console]
----
$ env:MULE_HOME="C:\Downloads\mule-enterprise-standalone-4.6.0"
----
+
...
[source,console]
----
$ export MULE_HOME=~/Downloads/mule-enterprise-standalone-4.6.0
----
