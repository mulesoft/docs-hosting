...
Start and stop on-premises Mule runtime engine to run applications on a host you manage. Registration with Anypoint Runtime Manager is optional. That registered model is Hybrid Standalone and is documented in xref:hybrid-standalone::index.adoc[Hybrid Standalone Runtimes], not in this on-premises set.
