...
a| Unregistered Mule doesn't connect to a control plane, so `anypoint.platform.base_uri` has no effect. See xref:on-premises::index.adoc[Run Mule Runtime Engine On-Premises].
|===
...
* xref:on-premises::runtime-installation-task.adoc[Download, Install, Configure, and Upgrade On-Premises Mule Runtime Engine]
