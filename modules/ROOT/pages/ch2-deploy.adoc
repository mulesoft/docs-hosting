...
Question: Do we need two topics - one each for shared and private?

You deploy apps to a private space using Runtime Manager.

== Deploy an App to a Private or Shared Space
...
*** <<app_properties,Configure app properties>>.
*** <<enable_tls_v1,Enable TLS v1 and TLS v1.1 with JVM arguments>>.
...
+
For private spaces, the administrator configures the domains for the space.
See xref:ps-config-domains.adoc[Enable Inbound Traffic to Your Apps Deployed to a Private Space].
+
After you select the domain, you can view the corresponding TLS context by clicking *View TLS Context*.
For private spaces, the administrator configures the TLS context for the space.
See xref:ps-config-domains.adoc[Enable Inbound Traffic to Your Apps Deployed to a Private Space].
* *Subdomain*: If the domain includes a wildcard, you can configure the subdomain.
+
Subdomains must start and end with an alphanumeric character and can contain only alphanumeric characters, hyphens, underscores, and periods.
+
