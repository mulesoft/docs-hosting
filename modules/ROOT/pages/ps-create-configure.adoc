...
--
. Take the following actions in the Anypoint Platform *Add Transit Gateway* page: 
+
--
.. Click *Next*.
...
--
+
The *Add Transit Gateway* page shows the progress for connecting to the resource share on AWS and creating the attachment.
+
When the attachment succeeds, the *Add Transit Gateway* page displays the *Attachment Created* message.
.. Click *Done*.
+
The *Connections* section on the *Transit Gateway* page shows the transit gateway ID and owner from AWS and indicates that the state is *Available*:

<screenshot>

// image::tgw-resource-share-available.png[Transit gateway added]

If the VPC attachment state is not `Attached to Private Network`,
click *Accept the attachment* link.

If the transit gateway addition fails, see xref:ps-troubleshoot.adoc#troubleshoot-transit-gateway-attachments[Troubleshoot Transit Gateway Attachments].
