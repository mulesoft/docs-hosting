...
. After the network creation succeeds, xref:ps-deploy.adoc[deploy an application] to test the private network.
...
. In *Static Routes*, enter the IP prefixes of the external networks to connect through the transit gateway.
...
When the VPC attachment succeeds, the *Transit Gateways* page displays the *Private space attached* message and the attachment state indicates that it's attached to Transit Gateway.
