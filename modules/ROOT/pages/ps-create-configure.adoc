...
.. Select the xref:ps-gather-setup-info.adoc#private-networkregion[region] where you want your apps to run.
...
+
*** *VPN*: Follow the steps in <<create-vpn>>.
*** *Transit Gateway*: Follow the steps in <<connect-tgw>>.
...
xref:ps-gather-setup-info.adoc#custom-tunnel-configuration[Custom]::
...
.. Specify the xref:ps-gather-setup-info.adoc#preshared-keys[pre-shared keys] (PSKs) for each VPN tunnel.
