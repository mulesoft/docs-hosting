...
*Last update* 02-Mar-22

Added all the endpoints from https://github.com/mulesoft/rtf-management-service/blob/master/assets/raml/public/api.raml[GitHub].

* *Private Spaces*
** Get the list of private spaces in the organization.
** Create a private space.
** Delete the private space.
** Get the private space. *What does this mean?*
** Update an existing private space.

* *Connections*
** Get the list of connections inside the private space.
** Create a private space connection. *What is this?*
...
** Get list of TGW connections associated with a private space.
** Create a private space TGW connection.
** Delete the private space TGW connection.
** Update the private space TGW connection.
** Update routes for a TGW connection.
** Get the routes configured for the TGW connection.
** Get the TGW routes configured for a private space.
** Get list of TGW connections assocated with an organization.
** Remove private space TGW connection.
** Get TGW routes configured for a gateway.

* *Log Forwarding*
** Get the log forwarding configuration information.
** Update the log forwarding configuration.
** Send test message to validate the end-to-end connectivity.
** Get the supported external log forwarding schemas.
** Get the specific configuration of a supported external log forwarding schema.
** Get the log forwarding config information.
** Update the Anypoint log forwarding config.
** Deletes external and Anypoint Monitoring log forwarding configuration.
...
** Get capacity information about the runtime fabric for an environment.
** Update existing fabric.
** Get a detailed health check for the Runtime Fabric.
** Get the list of Runtime Fabric Groups.
** Get the list of environment associations.
** Create a new environment association.
** Update an existing environment association.
** Retrieve short-lived credential to allow pulling Runtime Fabric Docker images.
** Retrieve properties related to image registry.

* *Upgrade Compatibility* 
** Get pre-upgrade compatibility check status and optionally limit the scope of the pre-check operation.
** Run the pre-upgrade compatibility checks.

* *Mule Runtime*
** Retrieve all supported Mule versions.
+
The first version returned is the most recent, recommended version, and the last version returned is the oldest supported version
** Retrieve a single name. *what?*
** Retrieves tags for a name. *what?*
** Retrieve a single name for a name. *what?*
...
** Download the latest Runtime Fabric agent helm chart.
...
*THIS IS THEORETICAL, BASED ON CH 1.0* 
...
* Manage private spaces, VPNs, and transit gateway attachments:
** Create, update, list, and remove a VPC.
** Create or delete a VPN connection.
** Create or delete a transit gateway attachment.
