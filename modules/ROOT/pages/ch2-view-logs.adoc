...
* To view logs for past deployments, click the *Past deployments* drop-down list and select the deployment.
...
and log levels.
+
To change any of these elements, click *X* in the *Search* field to exit the search results and enter a new term.
...
<screenshot>
+
Click a replica name to filter by replica?
+
To clear the replica filter, click the *X* button for the selected replica.
...
CloudHub 2.0 downloads the log output, using the current filters, to a text file in the format *_TBD_*.

// `__configID__.-__Date__Replica__.log`.
