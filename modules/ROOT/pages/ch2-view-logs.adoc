= View Log Data for Deployed Apps
...
CloudHub 2.0 stores up to 150 MB of log data per application, or up to 30 days, whichever limit is reached first.
...
+
The logs page displays all logs for the most recent deployment, with the latest messages at the bottom.
+
--
* To search within the currently displayed log, enter the search term in the *Search* field and press Enter.
+
...
* To view logs for past deployments, click the *Deployed* dropdown and select the deployment.
+
<replace screenshot>
+
...
+
Click the *Deployed* dropdown and select the deployment.
+
<replace screenshot>
+
// .The arrow shows the dropdown to filter log output by deployment.
...
+
Click *Latest Messages*, select the date range, and optionally time, from the dropdown, and click *Apply*.
+
<screenshot>
+
The *Latest Messages* button changes to show the selected date and time range.
...
+
--
** *Fatal*: List only fatal messages for when an application fails
** *Error*: List only error messages, such as when an exception occurs.
** *Warn*: List warning messages
** *Info*: List informative messages
** *Debug*: List debugging messages
--
+
[NOTE]
Some log levels might not be available depending on the dynamic log level setting for the deployment.
+
The *Log Levels* button changes to show the selected number of log levels displayed.


== Download Logs

To download logs based on the current filters, click *Download*.
...
CloudHub 2.0 downloads the log output to a text file in the format `__configID__.-__Replica__.log`.
...
* xref:ch2-view-diag.adoc[View Diagnostic Data for Deployed Apps]