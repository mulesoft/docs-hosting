...
For applications deployed to CloudHub 2.0, you can use Runtime Manager to set hidden properties.
...
For sensitive information, you can flag these properties as hidden so that, after they are entered and saved in Runtime Manager, their values are neither visible in the console nor passed between the console and CloudHub.
...
== Protect Properties in Runtime Manager

If you protect a property, users can't view or retrieve the property.
...
After the values are committed and the application is uploaded, the safely hidden properties are not displayed on the console or sent and received between the console and CloudHub.
...
See https://docs.mulesoft.com/mule-runtime/4.3/secure-configuration-properties[Secure Configuration Properties^] (link out from Beta docs) for more information on creating and using secure configuration properties.
...
If you don't want to encrypt properties for Mule applications that are deployed only to CloudHub, you can instead flag those properties as safely hidden properties in the Mule application's `mule-artifact.json` file's `secureProperties` entry.
