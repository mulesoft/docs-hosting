...
_Private DNS targets_ can can only be reached from inside a private network, or through VPN or transit gateway connections. Use it with internal DNS or allowlists in your connected external networks, so clients in those networks may make requests to your applications.
