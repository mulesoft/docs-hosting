...
+
*_Where are log levels configured?_*
+
...
. Select to forward ingress load balancer logs to the configured external service.
+
This option is available only if a destination service is configured in *Application Log Forwarding*.
. Select the default log level from the dropdown list: 
+
--
[%header,cols="20a,60a"]
...
+
.. Enter the IP address and select the log level.
.. Click *Add IP Address* to enter additional custom log levels.
.. To remove a custom log level, click trash can icon (*Delete*) for the entry.
// +
