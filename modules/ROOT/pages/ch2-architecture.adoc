...
For example, if you deploy an application named `myapp` to Canada (Central), the domain used to access the application is `myapp-_uniq-id_._shard_.ca-c1.cloudhub.io`.
...
| US Gov West |usg-w1.gov| `myapp._uniq-id_._shard_.usg-w1.gov.cloudhub.io`
3+h| EU Control Plane Regions 
| EU (Ireland) |ir-e1.eu1| `myapp._uniq-id_._shard_.ir-e1.eu1.cloudhub.io`
| EU (Frankfurt) |de-c1.eu1 | `myapp._uniq-id_._shard_.de-c1.eu1.cloudhub.io`
|===
...
Deploying to a region also affects your internal and external worker DNS address. For example, if you are using the US control plane and deploy to the Ireland region, the DNS records for internal and external IP addresses are `mule-worker-myapp._uniq-id_._shard_.ir-e1.cloudhub.io` and `mule-worker-internal-myapp._uniq-id_._shard_.ir-e1.cloudhub.io`.
