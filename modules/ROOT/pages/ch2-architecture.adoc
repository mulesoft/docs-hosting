...
image::ch2-architecture.png[CloudHub 2.0 Architecture]
...
[NOTE]
You can view the live status and detailed service history for Runtime Manager, platform services, and CloudHub on https://status.mulesoft.com/[status.mulesoft.com] for the default (US hosted) Control Plane and https://eu1-status.mulesoft.com/[eu1-status.mulesoft.com] for the EU Control Plane.
...
see xref:index.adoc[CloudHub 2.0] and xref:ps-deploy.adoc[Deploy Apps to Private Spaces].
...
They coordinate deployment of applications, monitor integrations, provide analytics data, store application data, run scheduled jobs, and more. Many of these services are also exposed through the xref:ch2-api.adoc[CloudHub 2.0 REST API].

== CloudHub 2.0 Replicas

_Replicas_ are dedicated instances of Mule runtime engine that run your integration applications on CloudHub.
...
The metaspace limit for apps deployed to CloudHub is currently 256 MB, regardless of the replica size.
...
See xref:cloudhub-fabric.adoc#worker-scale-out[Worker Scaleout] and 
https://docs.mulesoft.com/mq/[Anypoint MQ^] (Link out from Beta docs).

CloudHub monitors replicas to verify that they are operating correctly.
If you enable automatic restarts, CloudHub also automatically restarts applications, if necessary.


== Shared Global Regions

CloudHub provides the ability to deploy apps in different regions of the world: North America, South America, the European Union, and Asia-Pacific.
...
* The shared global region is a multitenant cloud of virtual machines.
+
...
All of CloudHub platform services, from load balancing to the API layer, have at least one, built-in layer of redundancy and are available in at least two data centers at all times.
All data centers are at least 60 miles apart. This redundancy ensures that even if there is a data center outage, the platform remains available.

=== Intelligent Healing

CloudHub monitors the replicas for problems and provides a self-healing mechanism to recover from them.
If the underlying hardware suffers a failure, the platform migrates your application to a new replica automatically.
...
While your application update is deploying, CloudHub keeps the old version of your application running.
...
xref:cloudhub-fabric.adoc#worker-scaleout[replica scaleout] features.

==== Scale Out and Data Center Redundancy

With xref:cloudhub-fabric.adoc[clustering], you can add multiple replicas to your application to make it horizontally scale.
...
You can also xref:ps-protect-app-props.adoc[protect application property values].
