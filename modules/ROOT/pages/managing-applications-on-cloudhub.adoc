...
:page-aliases: runtime-manager::managing-cloudhub-applications.adoc,runtime-manager::managing-applications-on-cloudhub.adoc
...
After you deploy an application to xref:deploying-to-index.adoc[CloudHub], you can use the Runtime Manager to manage your application at runtime.

Depending on if your application is deployed to xref:deploying-to-index.adoc[CloudHub] or to xref:deploying-to-your-own-servers.adoc[your own servers], there is a different set of possible actions you can perform.

[NOTE]
This document refers to applications that are deployed through the Runtime Manager console to xref:index.adoc[CloudHub]. To view the basic management tools that are common to all types of deployments, see xref:managing-deployed-applications.adoc[Manage Deployed Applications]. See xref:deployment-strategies.adoc[Deployment Options] to understand the different deployment scenarios that the platform offers.
...
What this dashboard displays depends on if your application is deployed on xref:index.adoc[CloudHub] or on xref:managing-applications-on-your-own-servers.adoc[Mule].
...
CloudHub supports allocating a static IP for applications so that they can be allowlisted for other services. To enable a static IP for your application, go to the *Static IPs* tab on your application settings page, then enable the *Use Static IP* checkbox. A static IP is allocated to your application and displayed in the table below; it will be used by the application when it restarts. If your application is not running, you can pre-allocate a static IP before starting the application. For details, see xref:deploying-to-index.adoc#static-ips-tab-settings[Static IPs Tab Settings].
