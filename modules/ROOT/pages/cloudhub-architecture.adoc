...
Manage CloudHub with the Anypoint Runtime Manager console in Anypoint Platform. You can also xref:deploying-to-index.adoc[deploy] to it directly from Anypoint Studio, via the xref:cloudhub-api.adoc[CloudHub REST API] or via the xref:anypoint-cli::index.adoc[Anypoint Platform command-line interface].
...
|image:blue-1.png[] |<<Runtime Management Console>> |User interface that enables you to deploy and monitor integrations, and configure your account

|image:blue-2.png[] |<<Platform Services>> |Shared CloudHub platform services and APIs, which includes CloudHub Insight, alerting, logging, account management, virtual private cloud/secure data gateway, and load balancing

|image:blue-3.png[] |<<Global Worker Clouds>> |An elastic cloud of _workers_, Mule instances that run integration applications
...
For more about creating and deploying integration applications, see xref:index.adoc[CloudHub] and xref:deploying-to-index.adoc[Deploy to CloudHub].
...
Through this same console you can xref:deploying-to-index.adoc[deploy to CloudHub] as well as to xref:deploying-to-your-own-servers.adoc[other registered servers]. You can also xref:managing-deployed-applications.adoc[manage deployed applications].
...
The metaspace limit for apps deployed to CloudHub is currently 256 MB, regardless of the worker size.
...
For more information about deploying applications to different regions, refer to xref:deploying-to-index.adoc[Deploy to CloudHub].
