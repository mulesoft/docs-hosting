...
This guide is for cloud deployments using the cloud-based version of Runtime Manager. See xref:deployment-strategies.adoc[Deployment Options] for information about different deployment scenarios.
...
After a static IP has been allocated for your application, the IP address is visible in the *Static IPs* tab in the application settings. For more information about static IPs, see xref:deploying-to-index.adoc#static-ips-tab-settings[Static IPs Tab Settings].
...
If you need to allow the IP addresses for the communication between the xref:runtime-manager-agent.adoc[Runtime Manager Agent] and the Runtime Manager console, see xref:rtm-agent-allowlists.adoc[Ports, IP Addresses, and Hostnames to Allow].
...
* xref:developing-applications-for-index.adoc#providing-an-external-http-https-port[Providing an External HTTP/HTTPS Port when deploying to CloudHub]
