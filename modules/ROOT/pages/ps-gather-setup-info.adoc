...
Use the following table to gather the information you'll need to set up your private space.
...
[stripes="even",frame="sides",cols="15a,8a,9a,10a,15a"]
...
|<<Remote IP Address>> 4+|
.5+|Routing Type .2+| <<Requirements for Dynamic VPN Connections,Dynamic>> | <<Remote ASN>>  2+|
                              | <<Local ASN>>   2+|
4+| *_OR_*
                 .2+| <<Requirements for Static VPN Connections,Static>>  | <<Static IP Prefixes>> 2+|
                              | <<Local ASN>>          2+|
