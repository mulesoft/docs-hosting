...
== Information Required to Set Up a Private Space (List)
...
(Optional) |<<IP Addresses>> 3+|
|<<Domains>>  3+|
6+|*Virtual Private Network Setup*
.13+||<<Remote IP Address>> 4+|
.2+|<<Static VPN Connection Requirements,Static Routing Type>>| <<Static IP Prefixes>> 3+|
                                                                                |<<Local ASN>> 3+|
^.^|*_OR_* 4+|
.2+| <<Dynamic VPN Connection Requirements,Dynamic Routing Type>> | <<Remote ASN>> 3+|
|<<Local ASN>> 3+|
 .4+|Custom Tunnel Configuration (Optional)   .2+| Tunnel 1 | <<Inside IP CIDRs>> 2+|
                                                | <<Pre-shared Keys>> 2+|
...
The domain provided for your application is based on the region to which your application is deployed.
