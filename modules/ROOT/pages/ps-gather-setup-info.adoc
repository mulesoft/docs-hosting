...
== (Better) Table: Information Required to Set Up a Private Space


[cols="1,16a,13a,19a,20a,30a"]
...
.5+|Routing Type .2+|  <<Static VPN Connection Requirements,Static>>  | <<Static IP Prefixes>> 2+|
                                                                                |<<Local ASN>> 2+|
|*_OR_* 3+|
.2+| <<Dynamic VPN Connection Requirements,Dynamic>> | <<Remote ASN>> 2+|
|<<Local ASN>> 2+|
.6+|Tunnel Configuration |Automatic 3+|
| *_OR_* 3+|
 .4+<.^|Custom   .2+| Tunnel 1 | <<Inside IP CIDRs>> |
                                                | <<Pre-shared Keys>> |
              .2+| Tunnel 2 | <<Inside IP CIDRs>> |
                                                 | <<Pre-shared Keys>> |
6+|*VPN Connection Setup*
.3+||<<Supported Gateway Devices,Device Vendor>>    4+|
|<<Supported Gateway Devices,Device Platform>>  4+|
|<<Supported Gateway Devices,Device Software>>  4+|
...
== Table: Information Required to Set Up a Private Space

The following table summarizes the information required to set up a private space.
For details about each requirement, see the sections that follow.


[stripes="even",frame="sides",cols="15a,15a,15a,15a,40a"]
.Private Space Configuration Form 
|===
5+|
...
.5+|Routing Type .2+|  <<Static VPN Connection Requirements,Static>>  | <<Static IP Prefixes>> 2+|
                              | <<Local ASN>>   2+|
^.^| *_OR_* 3+|
.2+| <<Dynamic VPN Connection Requirementss,Dynamic>> | <<Remote ASN>>  2+|
                              | <<Local ASN>>          2+|
.6+|Tunnel Configuration |Automatic 3+|
^.^| *_OR_* 3+|
                       .4+|Custom   .2+| Tunnel 1 | <<Inside IP CIDRs>> |
...
|<<Supported Gateway Devices,Device Vendor>>    4+|
|<<Supported Gateway Devices,Device Platform>>  4+|
|<<Supported Gateway Devices,Device Software>>  4+|
5+|*Internal DNS Setup*
|<<IP Addresses>>    4+|
|<<Domains>>  4+|
5+|
|===
...
== List: Information Required to Set Up a Private Space
...
** Routing Type
+
***  <<Static VPN Connection Requirements,Static>>
+
**** <<Static IP Prefixes>>: `10.0.0.0/8`
**** <<Local ASN>>: `64512`
*** <<Dynamic VPN Connection Requirements,Dynamic>>
+
**** <<Remote ASN>>: `65001`
**** <<Local ASN>>: `64512`
...
The _remote ASN_ (Autonomous System Number is the collection of routing prefixes that correspond to your backend.
...
The _local ASN_ is a private ASN (64512–65534) to assign to the Anypoint Platform side
...
two ends of a connection for the VPN tunnel.
...
The _IP addresses_ are up to three IP addresses of your internal DNS server.
...
_Domains_ are the domains to be resolved by the internal DNS server.

The domain is the common portion of your internal domain name, such as `mulesoft.com`.
...
See <<Dynamic VPN Connection Requirements>>.
...
See <<Static VPN Connection Requirements>>.
