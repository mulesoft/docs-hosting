= Manage App Properties on CloudHub 2.0
...
In a CloudHub 2.0 deployment, you can add properties to an application by using either Anypoint Runtime Manager or a properties file.
...
The easiest way to load properties to applications deployed to CloudHub 2.0 is to use the *Properties* tab in Anypoint Runtime Manager.
...
Although this method is not the best practice on CloudHub 2.0 (because CloudHub 2.0 *Properties* tab values override any with the same name in the file), you can work around this by changing options in the Mule app.
...
* https://docs.mulesoft.com/runtime-manager/secure-application-properties[Safely Hide Application Properties^] (Link out from Beta docs)