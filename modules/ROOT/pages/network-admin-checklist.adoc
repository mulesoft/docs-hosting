...
|Region|The region to deploy your apps to|`us-west-2`| <<Region>>
...
Apps use multiple IP addresses.
For each app, allocate 10 IP addresses. 
...
Ensure that you specify CIDR blocks from a private IP space, 
and that they don't overlap with any other CIDR blocks in your private spaces,
or any CIDR blocks in use in your corporate network.
