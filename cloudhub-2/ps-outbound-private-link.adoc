...
.. Availability Zone Ids: the id of the availability zones where the customer wishes to establish the private link. Make sure that the AZ Ids are the ones used by the private space (obtained in the first step here), and the VPC endpoint service is hosted in those AZ Ids.
...
----
+
If the VPC is successfully created, the API returns a `vpceId`.
