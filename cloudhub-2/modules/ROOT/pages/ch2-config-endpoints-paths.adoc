...
During disaster recovery, if an application has migrated to a new cluster, the cluster local DNS endpoint can fail. This is because traffic routed through the local DNS endpoint doesn't traverse the ingress load balancer, which is essential for directing traffic to the new cluster. However, using other available endpoints forces  traffic to pass through the ingress load balancer. The ingress load balancer finds out if a disaster recovery event is happening. It also knows if an application linked to a certain endpoint has been moved to a new cluster. It then routes traffic to the new cluster if needed.

During node upgrades and scaling, cluster local DNS endpoints are reachable as long as you deploy multiple application replicas.
