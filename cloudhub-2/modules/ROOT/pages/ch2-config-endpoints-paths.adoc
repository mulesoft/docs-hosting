...
An application communicate with another application via:
...
If you use the internal DNS name, traffic remains within the private space network. You can delete or omit the public endpoint when deploying an app. In that case, the app has only the private endpoint, which you can use for internal traffic.

If you use the cluster local endpoint is, the traffic doesn't leave the cluster. However, the cluster local endpoint is not high availability. During certain cluster operations such as disaster recovery, the endpoint can be unreachable. The cluster local endpoint allows only traffic within the same environment
