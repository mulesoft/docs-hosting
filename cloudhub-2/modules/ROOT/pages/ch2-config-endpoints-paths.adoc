...
If you use the cluster local endpoint, the traffic doesn't leave the cluster. However, the cluster local endpoint is not highly available. During some cluster operations such as disaster recovery, the endpoint can be unreachable. The cluster local endpoint allows traffic within the same environment only.
