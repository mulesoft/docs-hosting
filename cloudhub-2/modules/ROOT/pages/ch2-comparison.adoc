...
* In CloudHub 1.0, application names must be unique per control plane. In CloudHub 2.0, each name is unique per Business Group and Private Space. You can reuse a name in another Business Group or Private Space unless your organization has application name uniqueness enabled across Business Groups.
+
[NOTE]
====
When application name uniqueness is off (the default), you can use the same name in different Business Groups if the name is unique within each Business Group and Private Space. When the feature is on, names must be unique across all Business Groups in the organization.

The platform controls this behavior with the feature flag `app_mgr_quotas_app_name_uniqueness_enabled`. You cannot configure it. Contact MuleSoft Support to confirm whether your organization has the feature enabled.
====
...
* CloudHub applications have a two-minute grace period. CloudHub 2.0 applications have a five-minute grace period. For more information, see xref:ch2-deploy.adoc#grace-period[Grace Period].
