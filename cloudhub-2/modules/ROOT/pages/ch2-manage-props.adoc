...
Although this method isn't the best practice on CloudHub 2.0 (because the *Properties* tab values override any with the same name in the file), you can work around this by changing options in the Mule app.

[NOTE]
To mask secure property values on CloudHub 2.0, set property protection on the *Properties* tab in Runtime Manager. Because Runtime Manager properties override archive-bundled properties of the same name, masking values through `mule-artifact.json` or a configuration file doesn't take effect on CloudHub 2.0.
For information, see xref:ch2-protect-app-props.adoc[].
...
For more information about nondefault property placeholder options, see the https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/context/support/PropertySourcesPlaceholderConfigurer.html[Spring documentation on Property Placeholder options^].
...
== Access Environment and Organization IDs

CloudHub 2.0 provides environment and organization identifiers through environment variables. Reference these variables to retrieve the deployment environment ID and organization ID.
...
|`ENV_ID`|Environment ID where the application is deployed
|`ORG_ID`|Organization ID (business group ID) associated with the application
|===

To access these values, reference them using the standard property placeholder syntax: `${ENV_ID}` or `${ORG_ID}`.
