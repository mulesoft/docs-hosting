...
CloudHub 2.0 infrastructure can perform replica rebalancing to optimize application and infrastructure efficiency. Replica rebalancing involves gracefully moving and restarting replicas, as long as there are two or more replicas. At most, 30% of replicas for an application are rebalanced concurrently. This strategy keeps the application configuration the same, including the runtime version and infrastructure, for all types of restarts, except for restart after a security patching. For more information on security patching, see xref:ch2-patch-updates.adoc[].
