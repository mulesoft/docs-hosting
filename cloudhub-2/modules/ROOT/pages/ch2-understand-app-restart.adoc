...
Persistent object stores:: Configuring `persistent="true"` writes data to local disk. This works in Anypoint Studio and standalone Mule runtimes, but data doesn't persist across CloudHub 2.0 restarts. Use xref:object-store::index.adoc[Object Store v2] for persistent storage.
Local file operations:: The File connector and other file-based operations write to the replica's local disk. CloudHub 2.0 deletes these files when the app restarts.
Temporary files and caches:: App-generated files, custom logs, and local cache data don't persist.

To ensure data persistence in CloudHub 2.0:

* Use xref:object-store::index.adoc[Object Store v2] instead of persistent object stores for app state and synchronization data.
...
For more information about designing resilient applications, see xref:ch2-ha-dr.adoc[CloudHub 2.0 High Availability and Disaster Recovery].
...
In Private Spaces, CloudHub 2.0 uses Karpenter to manage node lifecycle. When Karpenter downscales nodes to remove underused capacity, CloudHub 2.0 restarts applications running on those nodes as replicas move to remaining nodes.

Restarts occur once per week at most. Each private space has a 4-hour consolidation window. CloudHub 2.0 randomizes the start time per space so it doesn't restart all spaces in a region at once. The 4-hour block can start anytime between Friday 10:00 PM and Saturday 2:00 AM in the Private Space region's local time zone, so restarts can occur anytime from Friday 10:00 PM through Saturday 6:00 AM.
