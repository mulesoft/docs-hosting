...
You can opt into early infrastructure upgrades to be applied earlier than the scheduled patching process, between the Release Available and Auto-Update dates stated in the xref:release-notes::cloudhub-2/cloudhub-2-runtimes-release-notes.adoc[monthly updates table]. In this way, you can choose a time that better suits your organization's needs. 

On average, this upgrade takes between thirty to sixty minutes to complete. If your upgrade is queued, it might take longer than thirty minutes. The impact of the upgrade process on running applications is consistent with the auto patching behavior described in the previous sections.

[NOTE]
This upgrade is irreversible. Ensure that you understand what the impact of the upgrade is, and choose when to trigger the upgrade based on your organization's needs before applying it in a production environment.
