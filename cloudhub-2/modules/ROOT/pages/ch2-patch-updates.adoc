...
When using Mule xref:ch2-clustering.adoc[clustering], a rolling update with a Mule runtime version that has a Hazelcast version change will generate a scenario when, at a certain point in time, the cluster will have more than one member with different Hazelcast versions. This may lead to application failure with a Hazelcast serialization error.
