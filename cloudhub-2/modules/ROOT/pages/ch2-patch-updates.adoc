...
On average, this upgrade takes between 30 to 60 minutes to complete. If your upgrade is queued, it can take longer than 30 minutes. In some cases, unexpected infrastructure delays, such as log processing retries, can extend the upgrade beyond this window. The impact of the upgrade process on running applications is consistent with the auto-patching behavior described in the previous sections.
...
include::release-notes::partial$mule-runtime/mule-lts-edge-release-cadence.adoc[tags=MuleIntro1A2]
...
After a new update occurs, you can check your xref:cloudhub-2::ch2-update-apps.adoc#replica-statuses[applications status state] in the *Status* column in Runtime Manager's *Applications* tab.

You can also check your application logs to verify if your application was deployed correctly and take any remediation actions in the event of some auto-minor update-related issue. For more information, visit xref:ch2-view-logs.adoc[].
