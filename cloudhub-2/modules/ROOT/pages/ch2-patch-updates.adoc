...
== Self-Triggered Infrastructure Upgrades

You can manually apply infrastructure upgrades earlier than the scheduled patching process. In this way, you can choose a time that better suits your organization's needs. 

[NOTE]
This upgrade is irreversible. Test in a lower environment before applying it in a production environment, and perform during low traffic windows.

=== Apply the Self-Triggered Upgrade via Runtime Manager
...
---- 
PATCH 
https:/anypoint.mulesoft.com/runtimefabric/api/organizations/{organizationId}/privatespaces/{privateSpaceId}/upgrade
----
