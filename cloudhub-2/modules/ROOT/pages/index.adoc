...
All features of CloudHub 2.0 are supported on all Control Planes with these exceptions:

Depending on the Control Plane you are deploying to: 

* Deleting the default egress traffic route, 0.0.0.0, can result in Anypoint Monitoring to not work in CloudHub 2.0.
* Endpoints URL format requires the region suffix or not before `cloudhub.io`.
