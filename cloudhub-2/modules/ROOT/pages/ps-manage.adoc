...
. For VPN connections created after [date], VPN logs are enabled by default upon creation.
. For existing VPN connections:
.. You can enable VPN logs via PATCH call to the following existing public endpoint:
+
`https://{{env}}.anypoint.mulesoft.com/runtimefabric/api/organizations/{{org-id}}/privatespaces/{{ps-id}}/connections/{{connection-id}}/vpns/{{vpn-id}}`.
+
.. Any updates you make to an existing VPN after [date] enabled the VPN logs to be directly accessible by MuleSoft Support Engineers.

[NOTE]
Enabling this feature speeds up the process for obtaining VPN logs from AWS, as the Support can download them directly, with no need to raise an AWS support ticket.
