...
+
During service updates or security patching, xref:mule-runtime::latest/global-settings-configuration.adoc#global-configurations-reference[CloudHub 2.0 waits five minutes] for existing schedules to complete. After a new replica is launched, the schedule runs at its next due. 
...
If clustering is enabled, CloudHub 2.0 ensures the schedule is executed on a single replica, regardless of how many replicas are running. Subsequent schedule executions might execute on a different replica. CloudHub 2.0 distributes multiple concurrent schedules across replicas.
* If clustering is not enabled, schedules run individually on all replicas.
* For a fixed frequency, the scheduler’s xref:cloudhub-2::ch2-manage-schedules.adoc#disable-enable-scheduler[minimum recommended startdelay is five seconds].
* If a schedule is not executed because the application is not running, Cloudhub 2.0 doesn't trigger it immediately when the application is started. The schedule is executed at its next due date.
