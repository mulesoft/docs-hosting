...
* Plan for high availability during network failures. CloudHub 2.0 infrastructure restarts due to hardware issues or scheduled maintenance, and security updates.
+
During service updates or security patching, xref:mule-runtime::global-settings-configuration.adoc#global-configurations-reference[CloudHub 2.0 waits five minutes] for existing schedules to complete. After a new replica is launched, the schedule runs at its next due. 
* Design your schedules to prevent duplicate data processing, as they can be triggered more than once due to the behavior mentioned previously.
If clustering is enabled, CloudHub 2.0 ensures the schedule is executed on a single replica, regardless of how many replicas are running. Subsequent schedule executions can occur on a different replica. CloudHub 2.0 distributes multiple concurrent schedules across replicas.
* If clustering isn't enabled, schedules run individually on all replicas.
* For a fixed frequency, the scheduler’s xref:cloudhub-2::ch2-manage-schedules.adoc#disable-enable-scheduler[minimum recommended start delay is five seconds].
* If a schedule isn't executed because the application isn't running, CloudHub 2.0 doesn't trigger it immediately when the application starts. The schedule is executed at its next due date.
...
* If you change schedulers, reupload the application to Exchange to make sure the expected scheduler behavior occurs.
