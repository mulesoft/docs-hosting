...
* By default, CloudHub 2.0 schedules run concurrently. The Scheduler triggers the execution of a schedule flow while a previously-triggered execution is still running. If you want to run schedules sequentially, set the `disallowConcurrentExecution` property as `true` in the xref:mule-runtime::scheduler-concept.adoc#props_scheduler[scheduler] configuration file. 
