...
** If the value is a placeholder (such as `${cronExpression}`) that is provided at the time of deployment, the exact value of the placeholder doesn't appear in the user interface.
...
This happens because the recreate deployment model turns off the previous instances of the application and starts them again in a new cluster. The previous record of the last run of the scheduler is not saved. In this ase, Mule runs the scheduler unless it is a fixed-date frequency. However,the rolling update deployment model saves the previous configuration.
