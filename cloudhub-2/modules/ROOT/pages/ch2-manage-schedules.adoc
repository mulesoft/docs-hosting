...
+
During service updates or security patching, xref:mule-runtime::global-settings-configuration.adoc#global-configurations-reference[CloudHub 2.0 waits five minutes] for existing schedules to complete. After a new replica is launched, the schedule runs at its next scheduled time. 
