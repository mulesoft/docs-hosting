...
Additionally, the value isn't viewable or retrievable by any user.
CloudHub 2.0 resolves the property internally at runtime.
These protected application values are encrypted and stored in the Anypoint Security secrets manager, which, in turn, is encrypted per user organization. Because the values are encrypted, you can't view them in Anypoint Security secrets manager or Anypoint Runtime Manager.

[NOTE]
====
On CloudHub 2.0, secure-property masking is configured in Anypoint Runtime Manager on the *Properties* tab (property protection).

If you're migrating from CloudHub 1.0, the `secureProperties` setting in `mule-artifact.json` doesn't mask values on CloudHub 2.0. To mask values, set property protection on the *Properties* tab, as described in the following sections.
Runtime Manager properties override archive-bundled properties of the same name.
====
