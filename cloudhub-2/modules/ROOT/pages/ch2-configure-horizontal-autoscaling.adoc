...
[IMPORTANT]
This feature applies only to select customers in the US control plane who are opted in to the new pricing and packaging model. For more details, visit xref:general::pricing.adoc[]
...
* CPU-based HPA works for micro and micro memory-intensive xref:general::pricing.adoc#cloudhub-2-0-application-deployment[replica sizes] for customers on Integration - Starter.
