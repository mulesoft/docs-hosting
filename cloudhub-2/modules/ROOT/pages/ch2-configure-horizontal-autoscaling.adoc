...
+
 Integration-Advanced customers can select from `Micro`, `Micro.Mem`, and `Small` replica sizes. Customers on other tiers can select from `Micro`and `Micro.Mem` replica sizes. For more information, see general::pricing.adoccloudhub-2-0-application-deployment[CloudHub 2.0 Application Deployment].
. Click *Deploy Application*.
