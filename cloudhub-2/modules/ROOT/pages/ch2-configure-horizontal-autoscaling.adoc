...
+
Integration-Advanced customers can select from `Micro`, `Micro.Mem`, and `Small` replica sizes. Customers on other tiers can select from `Micro`and `Micro.Mem` replica sizes. For more information, see general::pricing.adoc#cloudhub-2-0-application-deployment[CloudHub 2.0 Application Deployment].
. Click *Deploy Application*.
...
* Scale-up can occur, at most, every 180 seconds. Up to 100% of the currently running replicas may be added until the maximum configured replicas are reached. There is no stabilization window for scaling up. The target scales up immediately after the metrics indicate a scale-up is needed. 
* Scale-down can occur, at most, every 15 seconds. Up to 100% of the currently running replicas may be removed, which means the scaling target can be scaled down to the minimum allowed replicas. The number of replicas removed is based on the aggregated calculations over the past 1800 seconds of the stabilization window.
...
* Non-reentrant applications that don't have built-in parallel processing such as batch jobs, scheduler applications without re-entrance and duplicate scheduling across applications, and low-throughput, high-latency applications with large requests are not a good fit for CPU-based HPA.
