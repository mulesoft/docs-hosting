...
You can also track when autoscaling events occurred through audit logs. Each time an application deployment scales, an audit log is published under the product `Runtime Manager`, by the `Anypoint Staff` user. The log has action set to `Scaling`, and the object is the application ID.
