...
[IMPORTANT]
This feature applies only to select customers who are opted in to the new pricing and packaging model. For more details, visit xref:general::pricing.adoc[]
...
Scale-up can occur, at most, every 180 seconds. During each period, up to 100% of the currently running replicas may be added until the maximum configured replicas is reached. There is no stabilization window for scaling up. The target is scaled up immediately after the metrics indicate a scale-up is needed. 

Scale-down can occur, at most, every 15 seconds. During each period, up to 100% of the currently running replicas may be removed, which means the scaling target can be scaled down to the minimum allowed replicas. The number of replicas removed is based on the aggregated calculations over the past 1800 seconds of the stabilization window.
