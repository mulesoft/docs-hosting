...
[NOTE]
====
This feature is available to organizations with an Enterprise License Agreement (ELA) or those opted into the new pricing and packaging model. If the Horizontal Pod Autoscaling option isn't visible in the Runtime Manager UI, contact your account team.
====
...
+
If you have the Anypoint Integration Advanced package, you can select from `Micro`, `Micro.Mem`, and `Small` replica sizes. If you have a different pricing plan, select `Micro` or `Micro.Mem` replica sizes. For more information, see xref:general::pricing.adoc#cloudhub-2-0-application-deployment[CloudHub 2.0 Application Deployment].
. Click *Deploy Application*.
