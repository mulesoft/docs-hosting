...
Scale-up can occur, at most, every 180 seconds. Up to 100% of the currently running replicas may be added until the maximum configured replicas are reached. There is no stabilization window for scaling up. The target scals up immediately after the metrics indicate a scale-up is needed. 
