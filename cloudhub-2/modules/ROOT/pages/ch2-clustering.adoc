...
[IMPORTANT]
====
CloudHub 2.0 clears local disk storage when an app restarts or redeploys. Because `persistent="true"` writes to local disk, CloudHub 2.0 doesn't preserve data across restarts.

In Anypoint Studio and standalone Mule runtimes, object stores retain data across restarts because the local file system persists. CloudHub 2.0 replicas use volatile disk storage.

To persist data across restarts, use xref:object-store::index.adoc[Object Store v2] to store data externally.
====

[NOTE]
====
If you enable both **Clustering** and **Use Object Store V2** on CloudHub 2.0, all Object Store access routes to Cloud Object Store v2, including internal runtime, policy, and connector usage. This can result in metered, billable usage even if your application doesn't explicitly use Object Store.
====

You don't configure apps to use Object Store in CloudHub 2.0. Mule 4 apps support Object Store v2, which you enable in Anypoint Runtime Manager.
...
You can enable and disable either or both of cluster mode and horizontal autoscaling in one of two ways:
...
For more information, see xref:on-premises::mule-high-availability-ha-clusters.adoc[].
...
* No need to configure your application to support xref:mq::index.adoc[Anypoint MQ].
* Application schedules are executed on all replicas independently.
* Object Stores are not shared between replicas.
* If the object is written to one replica, the other replicas won't see the object, unless Cloud Object Store is used.
...
|
* The application can experience some message queue latency.
...
* Application schedule execution can be missed if the application runtime is not available at the time of execution.
* If the data center in which your replica operates experiences an outage, CloudHub 2.0 automatically migrates your application to another availability zone. You can experience downtime during the migration; however, your message queues ensure zero message loss.
