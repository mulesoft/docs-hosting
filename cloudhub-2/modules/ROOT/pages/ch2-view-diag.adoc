...
include::partial$diagnostic-agent.adoc[tag=prerequisites]
...
. Click the *Analyze & Troubleshoot* dropdown. You have two options:
...
Mule Thread Dumps::
+
A thread dump is a snapshot of all the threads in a replica that indicates the current state of the server.

Diagnostic Information Analysis File (DIAF)::
+
include::partial$diagnostic-agent.adoc[tag=diafFile]
+
[NOTE]
...
include::partial$diagnostic-agent.adoc[tag=considerationsLimitations]
