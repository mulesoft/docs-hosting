...
The CloudHub 2.0 private spaces load balancing service accepts public traffic on the standard HTTPS port: `443`. You can also choose to accept HTTP traffic on the standard HTTP port: `80`. Shared spaces don't accept port `80`.
...
CloudHub 1.0 VPC AWS direct connect connections have been deprecated in CloudHub 2.0. Instead, you can use TGW attachments. Further, when you delete a private space that has a TGW attached, the transit gateway is preserved, and you can reattach it to a different private space.
