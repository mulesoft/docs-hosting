...
The private DNS target uses private static IP addresses. These IP addresses remain constant throughout the lifetime of the private space, making them suitable for use in internal DNS configurations or allowlists in your connected external networks. You can view the private DNS target IP addresses in the *Inbound IP Addresses* section of the private space network configuration.
