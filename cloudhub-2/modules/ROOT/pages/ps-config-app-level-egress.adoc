...
Application level egress rules are independent of private space level firewall rules. For an egress traffic to go through, it needs to pass both private space and application level control.
...
. In the *Egress Rules* section, use the dropdown list and select the desired protocol and destination.
. Click *Add Rule* to add new rules to the rule group.
+ 
You can delete rules by clicking the trash can icon.
...
. Select *Delete* from the *More actions* menu  image:more-icon.png[The more actions icon, 2%,2%]. 
...
To allow HTTPS connections to OSv2, an app-level egress rule group with the following rule:
...
Given the extra layer of validation, using the application level egress rules can introduce slight network latency delays to your applications' connections. To balance security and performance:

* Limit active rule groups to 60 in your private space.
