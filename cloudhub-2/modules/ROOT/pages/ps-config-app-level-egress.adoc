...
+ 
You can edit and rename custom rule groups. The generated firewall rule name (`<private_space_name>-<rule_group_name>`) can't exceed 53 characters.
...
Domains with identical IP addresses are handled together. Allowing one domain allows all identical domains.
====
...
The preceding list includes common endpoints. Depending on your use case, add Anypoint Platform hostnames and ports for your application to communicate with the online Anypoint Platform APIs and services that are managed by MuleSoft. See xref:mule-gateway::mule-gateway-hostnames.adoc[].
...
You can't use custom domains with private space internal DNS.
====
...
Application-level egress rules don't support AWS STS regional endpoints, such as `sts.{region}.amazonaws.com`.
The STS domain uses a 30-second TTL, and egress rules for this domain can cause intermittent connection failures.

Avoid configuring application-level egress rules for these scenarios.
For more information, see https://help.salesforce.com/s/issue?language=en_US&id=a02Ka00000llC4i[Known Issue: App-level Egress Rules with Low TTL Domains^].

When you enable an xref:ps-config-advanced.adoc#configure-aws-role[AWS service role], applications using the AWS S3 Connector or other AWS SDK-based connectors still require outbound access to the public regional STS endpoint.
Allow that traffic at the private-space firewall. Don't add an application-level egress rule for STS.
For details and a workaround, see xref:ps-config-advanced.adoc#aws-iam-role-sts-s3-connector-conflict[IAM Roles and AWS S3 Connector: STS Endpoint Behavior].


[IMPORTANT]
====
The `ap-southeast` region updates its DNS records more frequently than other regions. This behavior can cause stale DNS entries and intermittent connectivity issues. When deploying applications in this region:

* Implement connection retry logic with exponential backoff.
* Monitor DNS resolution errors in your application logs.
* Use shorter TTL values to reduce stale cache entries.
====
