...
. In the *Egress Rules* section, click *Add Rule* and select the desired protocol and destination to add new rules to the new custom rule group.
+ 
You can delete rules by clicking the delete button image:trash-icon.png[Delete button, 2%,2%].
...
+
[NOTE]
Once you delete a custom rule group, any applications using it immediately starts using the default egress rule group instead.
...
. Use the dropdown list to select the egress ruleset you want to apply to your application. 
+
...
To set up connections to Anypoint MQ or OSv2, and access external endpoints, such as `anypoint.mulesoft.com` , follow the steps provided in this section.
