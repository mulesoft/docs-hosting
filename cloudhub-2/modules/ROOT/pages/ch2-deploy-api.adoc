...
To configure protocols other than HTTP or HTTPS, set up TCP inbound rules to manage traffic for your private space. You can then use the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/amc-application-manager/minor/4.0/console/method/%231158/[Application Manager API] to deploy an app using TCP. 
...
For incorrect port configurations, the following errors appear:

* `App <name> has leased the maximum number of ports.` occurs when you specify more than ten ports.
* `Ports must be within the supported range of 30500 - 32500` occurs when the port you specify is out of range.
* `Port 30501 is already in use by other owner` occurs when the port you specify is in use by another app.
