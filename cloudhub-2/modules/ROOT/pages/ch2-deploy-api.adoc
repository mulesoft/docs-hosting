...
You can access the app using VPN or a transit gateway and within the private space. Because the apps run on worker nodes, they cannot be accessed directly over the internet.  TCP endpoints use wildcard domain. The app-name or prefix is irrelevant. You can access applications on the endpoint: `*.<domain>.<region>.tcp.cloudhub.io:<port>`. 
