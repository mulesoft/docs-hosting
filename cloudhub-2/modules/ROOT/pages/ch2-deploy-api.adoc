...
Configure these network-level rules for the firewall in your private space:
...
These examples show the API request and response when you use it to find available ports:
...
In the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/amc-application-manager/minor/4.0/console/method/%231158/[Application Manager API], use the `tcp` section under `deploymentSettings` to map your TCP port and deploy your app. The mapping uses the external port to expose `portNumber` and the app level-defined port `applicationPortNumber`. You can use the same value for both fields, and there are no restrictions requiring them to differ. You can configure up to 10 ports per application.

This example shows the request to deploy a new app using a TCP port mapping:
...
This example shows a request to update an existing app and its body with a new TCP mapping:
...
* You can access the app using VPN or a transit gateway and within the private space. Because the apps run on worker nodes, you can't access them directly over the internet.
TCP endpoints use wildcard domains. The app-name or prefix is irrelevant. You can access applications on the endpoint: `*.<domain>.<region>.tcp.cloudhub.io:<port>`.
+
A sample URL is `mllp-app-2-mpskt5.q8b5lv.usa-e2.tcp.cloudhub.io:30500`.

* You can't establish a connection directly to the TCP URL, such as: `corvette-azeiuc.internal-yvs7m6.usa-w1.tcp.cloudhub.io.".`
+
The TCP URL format has the structure: {application-url}.{environment}.tcp.cloudhub.io:{port}. You can't add "internal-" to the {environment}, as CloudHub 2.0 doesn't support internal endpoints for TCP applications.
