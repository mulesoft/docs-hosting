...
You can use a load balancer, euther cloud-based or on-premises, for applications deployed to different regions to provide a better disaster recovery strategy.
...
* The runtime plane region and the control plane region are loosely coupled due to a service-oriented architecture.
* Clustering allows you to configure HA while running the process on a single primary replica.
* Horizontal Pod Autoscaling (HPA) for warm standby optimizes usage by elastic scaling if a failover is required.
