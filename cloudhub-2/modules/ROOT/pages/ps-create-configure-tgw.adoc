...
For download and usage information about the Network Tools application, see https://help.salesforce.com/s/articleView?id=001115029&type=1[How To Use Network Tools Application].
...
To remove or delete the transit gateway from CloudHub 2.0, complete the steps in <<delete-tgw>> before deleting the transit gateway, transit gateway attachment, or resource share from the AWS console. Removing connections from the AWS console without removing them from CloudHub 2.0 disrupts the connection but does not remove it. 
