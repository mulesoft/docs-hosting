...
When you attach a transit gateway to a private space, Amazon Web Services (AWS) creates a _transit gateway attachment_ to represent the connection. You can attach the same transit gateway to on-premisis VPCs in your own AWS account to enable communication between private spaces and VPCs. Multiple private spaces can connect to an internal VPC.
...
[[troubleshoot-tgw]]
== Troubleshoot Unknown or Deleted States for the Transit Gateway

When the transit gateway connection is disrupted, the state of the transit gateway changes to `unknown` or `deleted`. Depending on the cause of the state change, you can fix the state of the transit gateway by deleting or detaching the transit gateway.

The `unknown` state occurs when:
...
The `deleted` state occurs when the transit gateway is deleted from the AWS console but not CloudHub 2.0. To fix this state, detach the transit gateway from the private space using the steps in <<detach-tgw>>.
