...
[IMPORTANT]
====
If your private-space firewall doesn't allow outbound traffic to that STS endpoint, connectivity tests for the AWS S3 Connector and other AWS SDK-based connectors fail, and applications hang or stop progressing.
Allow outbound HTTPS on port 443 to the regional STS endpoint for the region where your private space runs. Configure this in xref:ps-config-fw-rules.adoc[].
...
When you can't allow the public regional STS endpoint, set the `AWS_ENDPOINT_URL` environment variable for your application to the STS endpoint URL so the SDK uses it instead.
...
. Apply the changes and redeploy or restart the application so the runtime uses the new value.
