...
[IMPORTANT]
More verbose log levels affect performance.
...
If you have identity and access management (IAM) roles configured in AWS, associate the role with your private space. The private space receives the permissions from the IAM role in AWS and accesses AWS resources. To configure this feature in AWS: 
...
. Click *Save Changes* or *Discard changes*.
+
A unique service role name is generated. Use this role to configure identity and access management for AWS. Role generation takes a few minutes. If the role name doesn't appear, refresh the page.
...
When you enable the AWS service role for a private space, the cluster injects environment variables such as `AWS_STS_REGIONAL_ENDPOINTS=regional`.
The AWS SDK credential provider that this setup uses ignores custom STS endpoint settings in your Mule application and always uses the public regional STS endpoint, such as `sts.{region}.amazonaws.com`.

[IMPORTANT]
====
If your private-space firewall doesn't allow outbound traffic to that STS endpoint, connectivity tests for the AWS S3 Connector and other AWS SDK-based connectors fail, and applications stop responding.
Allow outbound HTTPS on port 443 to the regional STS endpoint for the region where your private space runs. Configure this outbound access in xref:ps-config-fw-rules.adoc[].

Don't use application-level egress rules for the STS domain. Those rules don't support this endpoint and can cause intermittent connection failures. See xref:ps-config-app-level-egress.adoc#limitations[].
====

If you can't open outbound access to the public regional STS endpoint, set the AWS_ENDPOINT_URL environment variable for your application to the STS endpoint URL so that the SDK uses that URL instead.

To apply this workaround:

. In Runtime Manager, open your application, and then go to *Settings* > *Properties*.
...
. To have the runtime use the new value, apply the changes, and then redeploy or restart the application.

For more information about setting environment variables, see xref:ch2-manage-props.adoc#example-using-properties-to-set-environment-variables[Example: Using Properties to Set Environment Variables].
