...
The keystore in PEM format uses TLS default values. PEM files are recommended over JKS files.
--
JKS (Java Keystore)::
+
...
[NOTE]
The PEM keystore type is reccomended over JKS files.
...
[NOTE]
====
CloudHub 2.0 private space ingress support for TLS cipher version 1.1 ends starting January 31, 2025. Don’t use the following TLS 1.1 ciphers after that date:
```
