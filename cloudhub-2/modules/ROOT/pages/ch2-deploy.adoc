...
When you stop, restart, update, redeploy, or terminate an application, CloudHub 2.0 tries to delete it from Mule with a timeout period of five minutes to wait for inflight messages to be delivered. After that period, CloudHub 2.0 shuts down the whole instance. This grace period applies to the application instance, not to the runtime. The runtime uses its own configuration for the gracefully shutdown, which is, by default, five seconds.
