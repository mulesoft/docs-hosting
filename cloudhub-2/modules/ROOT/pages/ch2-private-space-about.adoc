...
Private space network traffic metrics aren't available in Anypoint Monitoring. For application-level traffic metrics, see xref:monitoring:am-faq.adoc#vpc-traffic-metrics[Can I get VPC-level network traffic metrics from Anypoint Monitoring?].
