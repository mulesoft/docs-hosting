...
Use this reference table to select the appropriate CIDR block size for your private space:
...
[NOTE]
These restrictions also apply to the respective subdomains.

You can't add MuleSoft domains or public domains as private space internal DNS.

As a workaround, create CNAME records on your own DNS servers to redirect a permitted custom domain to the restricted domain you want to add. See https://help.salesforce.com/s/articleView?id=001119691&type=1[Cannot add certain domains to CloudHub VPC Internal DNS].

// To specify an internal domain, use the second-level domain name, such as `mulesoft.com`.

[NOTE]
Each domain can contain up to 253 characters. When configured with 10 IP addresses (IPs) per entry, the maximum supported capacity is 1940 entries. The system supports more than 1940 entries if you use shorter domain names or fewer IPs in entries.
