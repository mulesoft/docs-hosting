...
[NOTE]
In some edge cases for applications using the rolling update deployment model, long inflight requests are not completed during the grace period. In this scenarios, zero downtime isn't guaranteed, as the old instance of the application needs to be restarted during the upgrade.
