...
|image:blue-2.png[2 Runtime Manager,width=50%] |<<runtime-manager>> |User interface that enables you to deploy and monitor integrations, and setup the runtime plane
...
[NOTE]
You can view the live status and detailed service history for Runtime Manager, platform services, and CloudHub 2.0 on https://status.salesforce.com/products/Mulesoft[status.mulesoft.com^] for the default (US hosted) Control Plane and https://status.salesforce.com/products/Mulesoft[eu1-status.mulesoft.com^] for the EU Control Plane.
...
A runtime region is the geographic location where CloudHub 2.0 runs your application. You select it at deploy time. It is independent of control plane data residency: deploying to Canada (Montreal) from US Cloud does not give you Canada Cloud.

The region determines the DNS record and the load balancer for your application. The *Region* value is the token used in CloudHub 2.0 DNS records.

Private spaces use the same *Region* token:

* Public: `<app>.<space-id>.<region>.cloudhub.io`
* Internal: `<app>.internal-<space-id>.<region>.cloudhub.io`

For private space DNS records, see xref:ch2-networking-guide.adoc#dns-records[DNS Records].

include::partial$region-table-long.adoc[]

For example, if you deploy an application named `myapp` to Canada (Montreal) in a shared space on US Cloud, the domain used to access the application is `myapp-_uniq-id_._shard_.can-c1.cloudhub.io`.
...
For more on Anypoint Platform control planes, including EU Cloud, see
...
[NOTE]
In some edge cases of applications using the rolling update deployment model, long inflight requests aren't completed during the grace period. In this scenarios, zero downtime isn't guaranteed, as the old instance of the application needs to be restarted during the upgrade.
...
* https://status.salesforce.com/products/Mulesoft[status.mulesoft.com^]
* https://status.salesforce.com/products/Mulesoft[eu1-status.mulesoft.com^]
* https://www.mulesoft.com/sites/default/files/security-whitepaper.pdf[Anypoint Cloud Security & Compliance whitepaper^]
