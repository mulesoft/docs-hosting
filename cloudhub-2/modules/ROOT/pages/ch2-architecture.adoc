...
Organizations that use CloudHub 2.0 and use the Anypoint Integration pricing packages use replica sizing for application deployment. You can choose from the following replica sizes based on the application profile or size and display the equivalent in terms of vCores. Replica sizes with the suffix `.mem` are memory-intensive options with the same compute.

include::general::partial$pricing.adoc[]
