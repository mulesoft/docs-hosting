...
Runtime Manager retrieves these values through the Runtime Manager agent, which queries the `System` Java class on the remote server. By default, auto-discovery is disabled for security. When enabled, Runtime Manager stores an encrypted version of these properties in its internal database.
