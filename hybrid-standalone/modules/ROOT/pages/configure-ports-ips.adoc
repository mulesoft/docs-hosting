...
This page lists US Cloud, EU Cloud, and MuleSoft Government Cloud endpoints. For Canada Cloud, Japan Cloud, India Cloud, and Australia Cloud, allowlist the Platform URL, analytics ingest, and Runtime Manager hosts in xref:control-planes::control-plane-hostnames-allowlists.adoc[Control Plane Hostnames and Allowlists].


== Mutual TLS Authentication Endpoints

The Runtime Manager agent uses mutual TLS (mTLS) to authenticate directly with these endpoints:
...
The `*.ingest.mulesoft.com` endpoints handle OpenTelemetry metrics ingestion.

[IMPORTANT]
Configure your firewall or proxy to allow SSL passthrough for these endpoints. Don't enable deep packet inspection (DPI) or SSL decryption. For `+*.ingest.mulesoft.com+`, allow outbound TCP on port 8443 and port 443.
...
* **OpenTelemetry metrics ingestion**: Runtime Manager agent 2.7.0 or later

[%header,cols="30a,60a"]
|===
|URL |Description
|runtime-manager.anypoint.mulesoft.com| ARM management capabilities (*public-cert agents*)
|data-authenticator.anypoint.mulesoft.com| Metrics ingestion for arm-monitoring stack (*public-cert agents*)
|us1.ingest.mulesoft.com (port 8443)| OpenTelemetry metrics ingestion (agent 2.7.0+)
|eu1.ingest.mulesoft.com (port 8443)| OpenTelemetry metrics ingestion (agent 2.7.0+)
|===
