...
Account for the following factors when you design a clustered Hybrid Standalone topology:
...
Simultaneous node restarts:: Do not restart multiple cluster nodes at the same time, such as during operating system patching. Concurrent restarts can trigger cluster-membership eviction and distributed-lock partition migration, which can take applications offline.
...
Object Store persistence:: Object Store persistence does not support sharing the same database across different clusters. Plan your persistence strategy accordingly.
