...
== Working with Mule Applications in Server Groups
...
** If you update a Mule application (for example, from version 1 to version 2) while servers have `Unknown` status, you can't perform additional updates (such as from version 2 to version 3) until you either:
  *** Remove the unknown servers from the server group, or
  *** Start and reconnect the unknown servers to the control plane

* Deleting applications +
When you delete a Mule application from a server group, it removes the application from all servers in the group.

=== Manual Application Management (Not Recommended)
