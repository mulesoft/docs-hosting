...
Anypoint Platform enables you to host the control plane and runtime plane in different locations based on your security, data privacy, and compliance requirements. Choose from cloud-managed options or self-hosted environments to control where your data is stored, processed, and managed. Host components in MuleSoft-managed cloud regions or deploy them in your own data centers. Each option keeps your data segregated within the physical location required by your company's IT policies.
...
+
Some Anypoint Platform components are not supported in Anypoint Platform PCE. See the <<support-matrix-control-plane>> for details.

This table shows more details about our regional services:

include::cloudhub-2::partial$region-table-short.adoc[]
...
[%header,cols="7*a"]
|===
| Runtime Plane | US Cloud | EU Cloud | MuleSoft Government Cloud | Anypoint Platform PCE | Canada Cloud | Japan Cloud
| CloudHub 2.0 | Y | Y | N | N | Y | Y 
| CloudHub | Y | Y | Y | N | N | N
| Hybrid Standalone runtimes | Y | Y | Y | Y | N | N
